...
const BottomBar = (props: BottomBarProps) => {
    ...
    const handleTabChange = (tabID : 'chat' | 'history') => {
        setActiveTab(tabID);
        if (onTabChange){
            onTabChange(tabID);
        }
    ...
    return (
        <div className="absolute bottom-0 left-0 max-w-md bg-blue-600 rounded-t-[46px] flex justify-center items-center w-full h-24 gap-20 shadow-green-xl ">
            {tabs.map((tab,index) => (
                <a
                    key={index}
                    href={tab.href}
                ...
                >
                    <div className={`flex flex-col items-center justify-evenly text-neutral-50 gap-0 w-20 h-16 ${activeTab === tab.id ? 'text-turquoise-200' : ''}`}>
                        <Image
