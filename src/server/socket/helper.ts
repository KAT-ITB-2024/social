...
import type { SocketClientInServer, SocketServer } from '~/server/socket';
import { db } from '~/server/db'; // pastikan db adalah instance Drizzle ORM
...
export type ServerEventResolver<T> =
 ...
 *
 * On handler, you can access the socket client, the socket server, and the drizzle client.
...
>(
  ...
  handler: ({
    ...
    ctx: {
      ...
      drizzle: typeof db; // pastikan tipe yang sesuai untuk Drizzle ORM instance
    };
    input: z.infer<InputSchema>;
    ...
    socket.on(
      ...
      async (data: unknown, callback?: (response: SocketResponse) => void) => {
        ...
        > = input?.safeParse(data) ?? {
          success: true,
          data: undefined as z.infer<InputSchema>,
        };
        ...
        try {
          const result = await handler({
            ctx: {
              io,
              client: socket,
              drizzle: db, // ganti prisma dengan drizzle
            },
